...
def sample(
    logits: torch.Tensor, top_k: int = 0, top_p: float = 0.0, temperature: float = 1.0, vocab_size: Optional[int] = None
) -> torch.Tensor:
    ...
    assert logits.type() == "torch.cuda.FloatTensor", f"input logits should be floats. Got: {logits.type()}"
