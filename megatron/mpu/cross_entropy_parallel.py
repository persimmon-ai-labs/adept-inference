...
#
# Taken from https://github.com/HazyResearch/flash-attention/blob/main/flash_attn/losses/cross_entropy_parallel.py
# But we import from Megatron instead of from Apex
# HazyResearch is licensed under BSD 3.0
...
class SoftmaxCrossEntropyLossParallelFn(torch.autograd.Function):
    @staticmethod
    def forward(ctx, logits_parallel, labels, smoothing=0.0, ignored_index=-100, inplace_backward=False):
        """
        ...
        if world_size == 1:
            ...
            vocab_start_index, vocab_end_index = VocabUtility.vocab_range_from_per_partition_vocab_size(
                partition_vocab_size, get_tensor_model_parallel_rank(), get_tensor_model_parallel_world_size()
            )
            ...
            handle_lse = torch.distributed.all_gather_into_tensor(
                lse_allgather, lse_local.contiguous(), group=get_tensor_model_parallel_group(), async_op=True
            )
            handle_losses = torch.distributed.all_reduce(
                losses, op=torch.distributed.ReduceOp.SUM, group=get_tensor_model_parallel_group(), async_op=True
            )
            handle_lse.wait()
    ...
    def backward(ctx, grad_loss):
        ...
        grad_logits = xentropy_cuda_lib.backward(
            grad_loss, logits_parallel, lse, labels, ctx.smoothing, ctx.inplace_backward
        )
