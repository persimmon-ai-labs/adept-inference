...
class DistributedDataParallel(DistributedDataParallelBase):
    ...
    def allreduce_gradients(self):
        ...
        if self._grad_buffers is not None:
            args = get_args()
            for _, buffer_ in self._grad_buffers.items():
                if args.nan_grad_check:
                    buffer_has_nan = buffer_.data.isnan().any()
                    if buffer_has_nan:
                        print(socket.gethostname(), "has a NaN!")
        ...
        # Sync tensor parallel replicated params.
        # These parameters should be the same across tensor parallel ranks because they are initialized to be the same,
        # and the gradients going into them are the same. However, during the data parallel all-reduce, the gradients
        # for the different parameters are added together in different orders. Because floating point addition is not
        # associative, the resulting parameters can be slightly different.
        # To fix this, we just broadcast the result that ended up in the first tensor parallel rank to the other ones.
        tp_replicated_buckets = collections.defaultdict(list)
        # Pack the buckets.
        for param in self.module.parameters():
            if mpu.param_is_tensor_parallel_replicated(param):
                tp_replicated_buckets[param.main_grad.dtype].append(param.main_grad)

        # For each bucket, broadcast.
        for bucket in tp_replicated_buckets.values():
            coalesced = _flatten_dense_tensors(bucket)
            torch.distributed.broadcast(
                coalesced, src=mpu.get_tensor_model_parallel_src_rank(), group=mpu.get_tensor_model_parallel_group()
            )
