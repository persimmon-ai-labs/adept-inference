...
from megatron.mpu import set_tensor_model_parallel_attributes
...
class MixedFusedLayerNorm(torch.nn.Module):
    def __init__(
        ...
        fused_mix_prec_layer_norm_cuda = importlib.import_module(
            "megatron_fused_kernels.fused_mix_prec_layer_norm_cuda"
        )
    ...
    def forward(self, input):
        # Doing this shape check here is important because the two kernels below don't handle shapes the same way,
        # so we need to check that we're passing them what we expect.
        assert len(self.normalized_shape) == 1
        if input.shape[-1] != self.normalized_shape[0]:
            raise ValueError(f"Expected input shape ending in {self.normalized_shape=}, but got {input.shape=}")
